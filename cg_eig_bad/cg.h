...
#include "../haoran_code/common.h"
#include "../haoran_code/matrix.h"
#include "../haoran_code/abstractsolver.h"

namespace CG
{
  void pcgsolve(const Mat &A, const Vec &b, FLOAT tol, int maxit, const function<void(const Vec&, const Vec&, FLOAT, FLOAT, Vec&)> &precon, const Vec &_x0, SolverReturnValue &ret, const Vec &_xknown)
	{
		assert(A.n==A.m); assert(A.n==b.n);
		Vec &x0=ret.x; x0=_x0; Vec r0=b-A*x0;
		FLOAT bnorm=b.norm(), err=r0.norm()/bnorm; 
		FLOAT xnorm=_xknown.norm(), actualerr=(_xknown-x0).norm()/xnorm;
                FLOAT Anorm=(_xknown*(A*_xknown)), errAnorm=(_xknown-x0)*(A*(_xknown-x0))/Anorm;
		...
		while (1)
		{
			...
			FLOAT err=r0.norm()/bnorm; rm.push_back(err);
                        actualerr=(_xknown-x0).norm()/xnorm;
                        errAnorm=((_xknown-x0)*(A*(_xknown-x0)))/Anorm;
			precon(r0,x0,err,tol,mr);
			FLOAT nt=r0*mr;
			FLOAT beta=nt/t; t=nt;
			d0=mr+d0*beta;
			printf("CG Iteration #%d, relres %.16lf, actualerr(2-norm) %.16lf, actualerr(A-norm) %.16lf\n",i,printFloat(err),printFloat(actualerr),printFloat(errAnorm));
  ...
  SolverReturnValue pcg(const Mat &A, const Vec &b, FLOAT tol, int maxit, const AbstractSolver &precon, const Vec &x0, const Vec &xknown)
	{
		SolverReturnValue ret;
		pcgsolve(A,b,tol,maxit,[&precon](const Vec &b, const Vec &x, FLOAT err, FLOAT tol, Vec &ret) { ret=precon.solve(b,tol); },x0,ret,xknown);
		return ret;
        }
	
  SolverReturnValue pcg(const Mat &A, const Vec &b, FLOAT tol, int maxit, const function<Vec(const Vec&, const Vec&, FLOAT, FLOAT)> &precon, const Vec &x0, const Vec &xknown)
	{
		SolverReturnValue ret;
		pcgsolve(A,b,tol,maxit,[&precon](const Vec &b, const Vec &x, FLOAT err, FLOAT tol, Vec &ret) { ret=precon(b,x,err,tol); },x0,ret,xknown);
		return ret;
        }
	
  SolverReturnValue pcg(const Mat &A, const Vec &b, FLOAT tol, int maxit, const function<Vec(const Vec&, const Vec&, FLOAT)> &precon, const Vec &x0, const Vec &xknown)
	{
		SolverReturnValue ret;
		pcgsolve(A,b,tol,maxit,[&precon](const Vec &b, const Vec &x, FLOAT err, FLOAT tol, Vec &ret) { ret=precon(b,x,err); },x0,ret,xknown);
		return ret;
        }
	
  SolverReturnValue pcg(const Mat &A, const Vec &b, FLOAT tol, int maxit, const function<Vec(const Vec&, FLOAT)> &precon, const Vec &x0, const Vec &xknown)
	{
		SolverReturnValue ret;
		pcgsolve(A,b,tol,maxit,[&precon](const Vec &b, const Vec &x, FLOAT err, FLOAT tol, Vec &ret) { ret=precon(b,err); },x0,ret,xknown);
		return ret;
        }
	
  SolverReturnValue pcg(const Mat &A, const Vec &b, FLOAT tol, int maxit, const function<Vec(const Vec&)> &precon, const Vec &x0, const Vec &xknown)
	{
		SolverReturnValue ret;
		pcgsolve(A,b,tol,maxit,[&precon](const Vec &b, const Vec &x, FLOAT err, FLOAT tol, Vec &ret) { ret=precon(b); },x0,ret,xknown);
		return ret;
        }
	
	SolverReturnValue pcg(const Mat &A, const Vec &b, FLOAT tol, int maxit, const AbstractSolver &precon, const Vec &xknown)
	{
		SolverReturnValue ret;
		pcgsolve(A,b,tol,maxit,[&precon](const Vec &b, const Vec &x, FLOAT err, FLOAT tol, Vec &ret) { ret=precon.solve(b,tol); },Vec(A.m),ret,xknown);
		return ret;
	}
...
*/
AbstractSolver PCG(const Mat &A, const Vec &xknown)
{
  return AbstractSolver([A,xknown](const Vec &b, FLOAT tol, int maxit, const Vec &x0) {
            return CG::pcg(A,b,tol,maxit,[](const Vec &z) { return z; },x0,xknown);
	});
}
